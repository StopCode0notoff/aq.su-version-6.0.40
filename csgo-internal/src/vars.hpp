...
#pragma pack(push, 1)
	struct ragebot_settings_t {
		...
		//_VAR_DECL_(types::bool_t, quick_stop_in_air); //stopinair

		
		//_VAR_DECL_(types::int_t, resolver_mode); // убрать
	...
	struct antiaim_settings_t {
		...
		_VAR_DECL_(types::int_t, jitter_type);
	...
	private:
		//
		types::list_t<types::name_t> m_name{};
		//
...
//			_VAR_DECL_(types::int_t, resolver_mode);

			//	enum class e_resolver_mode : int { // убрать
		//		off,
			//	edge,
			//	anim,
			//	AVTO
		//	};
			//_VAR_DECL_(types::int_t, resolver_mode);
			//_VAR_DECL_(types::bool_t, ping_spike);
		...
		struct {
			...
			_VAR_DECL_(types::bool_t, defensive_enable)
		} exploits;

		struct {
			_VAR_DECL_(types::bool_t, enable);

			_VAR_DECL_(antiaim_settings_t, triggers[8]); // default, stand, move, jump, slowwalk, crouch
			_VAR_DECL_(types::int_t, fakelag);
			_VAR_DECL_(types::int_t, fakelag_limit);
		} antiaim;
		...
		struct {
			...
			_VAR_DECL_(types::color_t, prediction_color);
		} grenade_esp;
		...
		struct {
			...
			_VAR_DECL_(types::int_t, thirdperson_dist);
			_VAR_DECL_(types::int_t, world_fov);
			_VAR_DECL_(types::int_t, zoom_fov);
			_VAR_DECL_(types::bool_t, viewmodel_in_scope);
			_VAR_DECL_(types::bool_t, force_crosshair);
			_VAR_DECL_(types::bool_t, penetration_crosshair);
			_VAR_DECL_(types::int_t, aspect_ratio);
			...
			struct {
				_VAR_DECL_(types::bool_t, enable);
				_VAR_DECL_(types::int_t, sunset_angle_x);
				_VAR_DECL_(types::int_t, sunset_angle_y);
			} sunsetmode;

			_VAR_DECL_(types::bool_t, fullbright);
		} visuals;
		...
		struct {
			...
			//_VAR_DECL_(types::bool_t, watermark); // убрать
			_VAR_DECL_(types::bool_t, hitsound);
			_VAR_DECL_(types::int_t, hitsound_type);
			...
			_VAR_DECL_(types::bool_t, shared_esp)
		...
		struct {
			...
			_VAR_DECL_(types::name_t, custom_model_path); // убрать
		} skins;

		uint8_t end{};

		types::int_t dpi_scale = 0;
