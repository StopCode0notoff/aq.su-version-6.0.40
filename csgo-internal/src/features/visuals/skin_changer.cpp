...
STFI void update_knife(base_combat_weapon_t* weapon) {
	...
	//		if (auto original_item = SkinParser::GetWeaponInfo(old_definition_index); original_item && !original_item->icon.empty() && !replacement_item->icon.empty())
...
STFI std::string get_model(int team) {
	//if (!skin_changer->custom_model_path.empty()) {
		//return "csgo/materials/models/player/custom_player/" + skin_changer->custom_model_path;         // óáðàòü
	//}                             // óáðàòü
		if (!skin_changer->custom_model_path.empty()) {
			return "models/player/custom_player/" + skin_changer->custom_model_path;
		}
...
void skin_changer_t::on_frame_stage_notify(e_client_frame_stage stage) {
	...
	if (!skin_changer->custom_model_path.empty()) {
		std::string model_path = "models/player/custom_player/" + skin_changer->custom_model_path;
		int model_index = interfaces::model_info->get_model_index(model_path.c_str());


		if (model_index != -1) {
			globals->m_local->set_model_index(model_index);
			return;

			auto viewmodel = (base_viewmodel_t*)globals->m_local->viewmodel_handle().get();
			if (viewmodel) {
				int default_hands = interfaces::model_info->get_model_index("models/weapons/v_models/arms/v_models.mdl");
				viewmodel->model_index() = default_hands; 
			}
