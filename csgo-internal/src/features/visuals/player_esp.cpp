...
namespace esp {
	STFI void render_box(const player_entry_t& entry, bounding_box_t& box) {
		...
		auto ping_color = color_t{ 255, 255, 255, 255 };

		if (flags.get() != 0) {
			if (entry.m_friend_cheat == 1)
				text_arr.emplace_back(STR("astralquo"), color_t{ 254, 103, 49 }.modify_alpha(box.m_alpha));
				...
				text_arr.emplace_back(STR("ADMIN"), color_t{ 0, 255, 163 }.modify_alpha(box.m_alpha));
			else if (entry.m_friend_cheat == 4)
				text_arr.emplace_back(STR("Furcore"), color_t{ 115, 180, 255 }.modify_alpha(box.m_alpha));
			else if (entry.m_friend_cheat == 5)
				text_arr.emplace_back(STR("Floridahook"), color_t{ 115, 180, 255 }.modify_alpha(box.m_alpha));
			else if (entry.m_friend_cheat == 6)
				text_arr.emplace_back(STR("UNK"), color_t{ 115, 180, 255 }.modify_alpha(box.m_alpha));
		}

		if (flags.at(5) && entry.m_ping > 0 && !entry.m_dormant) {
			if (entry.m_ping > 0 && entry.m_ping < 60) {
				ping_color = color_t{ 0, 255, 0 };
			} else if (entry.m_ping > 60 && entry.m_ping < 120) {
				ping_color = color_t{ 255, 185, 0 };
			} else if (entry.m_ping > 120) {
				ping_color = color_t{ 255, 0, 0 };
			}

			text_arr.emplace_back(dformat(STR("{}ms"), entry.m_ping), ping_color);
		}
		...
		if (flags.at(6) && entry.m_breaklagcomp && !entry.m_dormant)
			text_arr.emplace_back(STR("LC"), color_t{ 255, 0, 0, 255 });

		if (flags.at(7) && entry.m_hittable && !entry.m_dormant)
			text_arr.emplace_back(STR("Hit"), flags_color);

		if (flags.at(8) && entry.m_has_c4)
			text_arr.emplace_back(STR("C4"), flags_color);
