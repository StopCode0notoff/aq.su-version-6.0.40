...
namespace esp {
		...
		if (source != vec3d{ 0, 0, 0 }) {
			...
			if (math::world_to_screen(source, center2d)) {
				...
				for (float angle = 0.f; angle <= f2PI; angle += step) {
					auto point3d = vec3d{ sin(angle), cos(angle), 0.f } * 18.5f * easings::out_quint(animation);
					vec2d point2d{};
	...
	STFI void penetration_crosshair()
	{
		if (!settings->visuals.penetration_crosshair || !globals->m_local_alive || !globals->m_is_connected) {
			return;
		}

		const auto screen_center = vec2d{ render::screen_width / 2.f, render::screen_height / 2.f };

		const color_t color = globals->m_can_penetrate ? ( 0, 255, 0, 255 ) : ( 255, 0, 0, 255 );
