...
#include "../../bytes/hitsound/hit5.h"
#include "../../bytes/hitsound/hit6.h"
#include "../../bytes/hitsound/hit7.h"
#include "../../bytes/hitsound/hit8.h"
...
STFI void play_hitsound() {
	...
	switch (settings->misc.hitsound_type) {
		case 0:
			...
			break;
		case 1:
			utils::play_sound_from_memory(hitsound_raw_5, sizeof(hitsound_raw_5), volume);
			break;
		case 2:
			utils::play_sound_from_memory(hitsound_raw_6, sizeof(hitsound_raw_6), volume);
			break;
		case 3:
			utils::play_sound_from_memory(hitsound_raw_7, sizeof(hitsound_raw_7), volume);
			break;
		case 4:
			utils::play_sound_from_memory(hitsound_raw_8, sizeof(hitsound_raw_8), volume);
			break;
		case 5:
			interfaces::engine->client_cmd_unrestricted(STRC("play buttons/arena_switch_press_02.wav"), 0);
			break;
		case 6:
			interfaces::engine->client_cmd_unrestricted(STRC("play resource/warning.wav"), 0);
			break;
	}
}
...
STFI std::string hitgroup_to_string(e_hitgroup hitgroup) {
	switch (hitgroup) {
		...
		//case e_hitgroup::neck: return STR("neck"); вернуть
...
STFI void print_damage_dealt(std::string name, int damage, int health_left, e_hitgroup hitgroup) {
	game_console->print_colored_id(dformat(STR("[$1astralquo$6] $2Hit $3{} $2for $3{}hp $2in $3{} $2({}hp left)\n"), name, damage, hitgroup_to_string(hitgroup), health_left));
...
STFI void print_damage_received(std::string name, int damage, int health_left, e_hitgroup hitgroup) {
	game_console->print_colored_id(dformat(STR("[$1astralquo$6] $2Hurt from $3{} $2for $3{}hp $2in $3{} $2({}hp left)\n"), name, damage, hitgroup_to_string(hitgroup), health_left));
						...
						if (fired_shot_hitgroup != hitgroup && globals->m_weapon != nullptr) {
							...
							if (end_pos.has_value()) {
								...
								if (penetration.m_did_hit && penetration.m_hitgroup == fired_shot_hitgroup)
									game_console->print_colored_id(STR("[$1astralquo$6] $2Damage mismatch due to $4reZolVer\n"));
								else
									game_console->print_colored_id(STR("[$1astralquo$6] $2Damage mismatch due to $3spread\n"));
							}
...
STFI void process_shot(shot_record_rage_t& shot) {
	...
	if (settings->misc.log_filter.at(3) && !shot.m_lag_record.m_player->is_alive()) {
		...
		game_console->print_colored_id(STR("[$1astralquo$6] $2Missed shot due to $3enemy death\n"));
		return;
	}
	...
	if (spread_miss) {
		if (settings->misc.log_filter.at(3)) {
			...
			game_console->print_colored_id(STR("[$1astralquo$6] $2Missed shot due to $3spread\n"));
		}
	} else {
		if (settings->misc.log_filter.at(3)) {
			cheat_logs->add(STR("Missed shot due to reZolVer"), cheat_logs_t::message_t::e_type::miss);
			game_console->print_colored_id(STR("[$1astralquo$6] $2Missed shot due to $4reZolVer\n"));
		}
		...
		if (globals->m_local_alive) {
			...
			if (it->m_confirmed && it->m_impacts.empty()) {
				if (settings->misc.log_filter.at(3)) {
					...
					game_console->print_colored_id(STR("[$1astralquo$6] $2Missed shot due to $3death\n"));
				}
			}
