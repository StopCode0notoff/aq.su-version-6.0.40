...
//#include "E:\source\v3.4\csgo_internal\sdk\entities.hpp"
...
namespace hvh {
	...
	__forceinline e_hitgroup hitbox_to_hitgroup(e_hitbox hitbox) {
		switch (hitbox) {
			...
			//case e_hitbox::neck: return e_hitgroup::neck; вернуть
	...
	__forceinline vec3d predict_local_player_origin(const vec3d& origin, const vec3d& velocity, const vec3d& old_velocity, int ticks, bool debug) {
		...
		//	(/*globals->m_local->velocity().z > 100.f ||*/ globals->m_local->velocity().z < -5.f))
		//	return;
		//else if (!Features::Ragebot.MenuVars.RagebotAutostopInAir && !(globals->m_local->flags() & FL_ONGROUND))
		//	return;  //stopinair
