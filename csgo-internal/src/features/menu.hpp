...
namespace menu {
	...
	static void begin_child(std::string name,/* float width = 0.f, */float height = 0.f) {
...
static void end_child() {
	...
	static void slider_int(std::string label, int* v, int min, int max, std::string format = STR("%d")) {
		ImGui::PushItemWidth(/*gui::dpi::scale*/(max_item_width));
		ImGui::Text(label.c_str());
		ImGui::SliderInt((STR("##") + label).c_str(), v, min, max, format.c_str(), ImGuiSliderFlags_NoInput);
		ImGui::PopItemWidth();
	}
	...
	static void slider_float(std::string label, float* v, float min, float max, std::string format = STR("%.2f")) {
		ImGui::PushItemWidth(/*gui::dpi::scale*/(max_item_width));
		ImGui::Text(label.c_str());
		ImGui::SliderFloat((STR("##") + label).c_str(), v, min, max, format.c_str(), ImGuiSliderFlags_NoInput);
		ImGui::PopItemWidth();
	}

	static void checkbox(std::string label, bool* v)
	{
	...
	static void combo_font(std::string label, int *value)
	{
		std::vector<std::string> elements = { STR("Verdana"), STR("Menu"), STR("Small pixel"), STR("RobotoFlex-BigRegular"), STR("RobotoFlex-Bold"), STR("RobotoFlex-BigBold"), STR("RobotoFlex-Regular"), STR("RobotoFlex-SemiBold"), STR("RobotoFlex-SmallBold") };
		ImGui::PushItemWidth(/*gui::dpi::scale*/ (max_item_width));
		ImGui::Text(label.c_str());
		if (ImGui::BeginCombo((STR("##") + label).c_str(), elements[*value].c_str())) {
			for (size_t i = 0u; i < elements.size(); ++i)
	...
	static void bar_settings(std::string name, incheat_vars::esp::bar_t& set, ImTextureID texture) {
		...
		if (set.value) {
			...
			if (ImGui::ButtonColorChanger(texture, (STR(".") + id).c_str(), ImVec2(20, 20)))
				ImGui::OpenPopup(ImGui::GetID(id.c_str()));
		}

		if (ImGui::BeginPopup(id.c_str())) {
			if (&set == &settings->player_esp.health) {
				checkbox(STRC("Custom color"), &settings->player_esp.override_health_color);
	...
	static void text_array_settings(std::string name, std::vector<std::string> elements, incheat_vars::esp::text_array_t& set, ImTextureID texture) {
		std::string id = STR("##") + name;
	...
	static void text_settings(std::string name, incheat_vars::esp::text_t& set, ImTextureID texture) {
