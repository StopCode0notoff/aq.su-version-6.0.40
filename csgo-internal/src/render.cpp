...
namespace render {
	...
	void merge_with_fontawesome(ImFontConfig& cfg, float size, float original_size, bool bold = false) {
		...
		//ImGui::GetIO().Fonts->AddFontFromFileTTF(bold ? STRSC("C:\\astralquo\\fonts\\fa-solid-900.ttf") : STRSC("C:\\astralquo\\fonts\\fa-regular-400.ttf"), size, &cfg, ranges);
		ImGui::GetIO().Fonts->AddFontFromFileTTF(STRSC("C:\\ProgramData\\astralquo\\fonts\\fa-solid-900.ttf"), size, &cfg, ranges);
		ImGui::GetIO().Fonts->AddFontFromFileTTF(STRSC("C:\\ProgramData\\astralquo\\fonts\\fa-brands-400.ttf"), size, &cfg, ranges);
	...
	void merge_with_chinese(ImFontConfig& cfg, float size, bool bold = false) {
		...
		ImGui::GetIO().Fonts->AddFontFromFileTTF(
				bold ? STRSC("C:\\ProgramData\\astralquo\\fonts\\Source-Han-Sans-CN-Bold.ttf") : STRSC("C:\\ProgramData\\astralquo\\fonts\\Source-Han-Sans-CN-Regular.ttf"),
				size, &cfg,
				ranges);
	...
	void init_fonts() {
		...
		io.FontDefault = fonts::menu_main = io.Fonts->AddFontFromFileTTF(STRSC("C:\\ProgramData\\astralquo\\fonts\\RobotoFlex-Regular.ttf"), dpi::scale(14.5f), &cfg, ranges);
		//merge_with_chinese(cfg, dpi::scale(14.5f), false);
		merge_with_fontawesome(cfg, dpi::scale(14.5f * icon_size_mod), dpi::scale(14.5f));

		fonts::menu_main_weapons = io.Fonts->AddFontFromFileTTF(STRSC("C:\\ProgramData\\astralquo\\fonts\\RobotoFlex-Regular.ttf"), dpi::scale(14.5f), &cfg, ranges);
		merge_with_weapons(cfg, dpi::scale(14.5f));

		fonts::menu_bold = io.Fonts->AddFontFromFileTTF(STRSC("C:\\ProgramData\\astralquo\\fonts\\RobotoFlex-Bold.ttf"), dpi::scale(14.5f), &cfg, ranges);
		//merge_with_chinese(cfg, dpi::scale(14.5f), true);
		merge_with_fontawesome(cfg, dpi::scale(14.5f * icon_size_mod), dpi::scale(14.5f), true);

		fonts::menu_small = io.Fonts->AddFontFromFileTTF(STRSC("C:\\ProgramData\\astralquo\\fonts\\RobotoFlex-Regular.ttf"), dpi::scale(14.f), &cfg, ranges);
		//merge_with_chinese(cfg, dpi::scale(14.f), false);

		fonts::menu_small_bold = io.Fonts->AddFontFromFileTTF(STRSC("C:\\ProgramData\\astralquo\\fonts\\RobotoFlex-Bold.ttf"), dpi::scale(14.f), &cfg, ranges);
		//merge_with_chinese(cfg, dpi::scale(14.f), true);

		cfg.GlyphExtraSpacing = { 0.25f, 0.f };
		fonts::menu_big = io.Fonts->AddFontFromFileTTF(STRSC("C:\\ProgramData\\astralquo\\fonts\\RobotoFlex-Bold.ttf"), dpi::scale(18.f), &cfg, ranges);
		//merge_with_chinese(cfg, dpi::scale(18.f), true);
		cfg.GlyphExtraSpacing = { 0.f, 0.f };

		fonts::menu_small_semibold = io.Fonts->AddFontFromFileTTF(STRSC("C:\\ProgramData\\astralquo\\fonts\\RobotoFlex-SemiBold.ttf"), dpi::scale(14.f), &cfg, ranges);
		//merge_with_chinese(cfg, dpi::scale(14.f), false);

		fonts::esp_default = io.Fonts->AddFontFromFileTTF(STRSC("C:\\Windows\\Fonts\\Verdana.ttf"), dpi::scale(14.f), &cfg, io.Fonts->GetGlyphRangesCyrillic());
		...
		fonts::esp_small = io.Fonts->AddFontFromMemoryTTF(smallest_pixel_7, sizeof(smallest_pixel_7), dpi::scale(10.f), &cfg, io.Fonts->GetGlyphRangesCyrillic());
		...
		/*if (dpi::_get_actual_scale() > 1.f) {
			fonts::esp_small = io.Fonts->AddFontFromFileTTF(STRSC("C:\\astralquo\\fonts\\RobotoFlex-Regular.ttf"), dpi::scale(11.f), &cfg, io.Fonts->GetGlyphRangesCyrillic());
		} else {
			SET_AND_RESTORE(cfg.FontBuilderFlags, ImGuiFreeTypeBuilderFlags_MonoHinting | ImGuiFreeTypeBuilderFlags_Monochrome);
			fonts::esp_small = io.Fonts->AddFontFromFileTTF(STRSC("C:\\astralquo\\fonts\\Smallest-Pixel7.ttf"), dpi::scale(10.f), &cfg, io.Fonts->GetGlyphRangesCyrillic());
		}*/

		fonts::menu_desc = io.Fonts->AddFontFromFileTTF(STRSC("C:\\Windows\\Fonts\\Verdana.ttf"), 16.f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
