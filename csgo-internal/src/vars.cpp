...
namespace incheat_vars {
	...
	utils::bytes_t settings_t::dump_settings(bool pretty) {
		json_t _json{};
...
#define __USERNAME__ STRS("StopCode")

#define __CFG_VERSION__ XOR32S(1)

#define __CFG_PATH__ dformat(STRS("astralquo\\settings\\{}"), __USERNAME__)
	...
	void settings_t::save(std::string_view id) {
		...
		write_file_bin(STR("astralquo\\settings\\") + id.data(), settings->dump_settings(true));
	}
	...
	void settings_t::load(std::string_view id) {
...

		{
			...
			gui::dpi::change_scale(gui::dpi::_scale);
			//}
		}
		...
		const auto& setting = read_file_bin(STR("astralquo\\settings\\") + id.data());
	...
	// MODELCHANGER
#define VARS_ADD_NAME(varname, def_value)       \
    m_name[&varname] = _VAR_GET_NAME(#varname); \
    varname = def_value
	//
...
#define VARS_ADD_COLOR(varname, r, g, b, a)       \
	...
	//VARS_ADD_BOOL(ragebot.weapons[group_index].settings[weapon_index].quick_stop_in_air, false); 
...
#define ADD_RAGE_WEAPON(group_index, weapon_index)                                              \
	...
	VARS_ADD_FLAGS(ragebot.weapons[group_index].settings[weapon_index].quick_stop_options, 0);  \
...
#define ADD_ANTIAIM_TRIGGER(group_index)                                       \
	...
	VARS_ADD_INT(antiaim.triggers[group_index].custom_pitch, 0);               \
	...
	VARS_ADD_BOOL(antiaim.triggers[group_index].randomize_desync, false);        \
	...
	VARS_ADD_INT(antiaim.triggers[group_index].jitter_type, 0);            \
	...
	void settings_t::init_visuals() {
		...
		VARS_ADD_INT(player_esp.ammo.font, 0);
		...
		VARS_ADD_INT(player_esp.health.font, 0);
...
#define ADD_CHAMS_SETTINGS(index)                                                   \
	...
	VARS_ADD_COLOR(player_esp.chams[index].overlays_colors[4], 255, 255, 255, 255);
		...
		VARS_ADD_COLOR(grenade_esp.prediction_color, 255, 255, 255, 255);
		...
		VARS_ADD_BOOL(visuals.sunsetmode.enable, 0);
		VARS_ADD_INT(visuals.sunsetmode.sunset_angle_x, 0);
		VARS_ADD_INT(visuals.sunsetmode.sunset_angle_y, -140);
		VARS_ADD_BOOL(visuals.fullbright, 0);
		...
		VARS_ADD_INT(visuals.thirdperson_dist, 50);
		VARS_ADD_BOOL(visuals.viewmodel_in_scope, false);
		VARS_ADD_BOOL(visuals.force_crosshair, false);
		VARS_ADD_BOOL(visuals.penetration_crosshair, false);
		VARS_ADD_INT(visuals.aspect_ratio, 0);
	}
	...
	void settings_t::init_misc() {
		...
		VARS_ADD_INT(movement.leg_movement, 0);
...
#define VARS_GET_NAME_(x) (x)
		...
		VARS_ADD_NAME(skins.custom_model_path, "");
	}
