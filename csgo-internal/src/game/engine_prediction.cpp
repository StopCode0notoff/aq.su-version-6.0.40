...
void engine_prediction_t::entry_t::store(sdk::user_cmd_t* cmd) {
	m_cmd = *cmd;				  // Вместо memcpy
	m_global_vars = *global_vars; // Вместо memcpy
	...
	if (auto viewmodel = (base_viewmodel_t*)globals->m_local->viewmodel_handle().get()) {
...
void engine_prediction_t::repredict(user_cmd_t* cmd) {
	if (!m_is_predicted || hvh::exploits->m_in_shift)
		return;
	...
	const auto curtime = TICKS_TO_TIME(hideshot ? globals->m_local->tickbase() - 8 :  globals->m_local->tickbase()); /*hideshot ? globals->m_local->tickbase() - 8 : */
...
struct cached_prediction_t {
	vec3d origin;
	vec3d velocity;
	int tickbase;
	int flags;
};
cached_prediction_t m_last_predicted_data;