...
private:
	...
	static constexpr float AIR_SPEED_THRESHOLD = 30.0f;
	static constexpr int MAX_SWITCH_COUNT = 5;			
	static constexpr float JITTER_BEGIN_ANGLE = 15.0f;	
	static constexpr int JITTER_CACHE_SIZE = 3;			

	int determine_initial_side(sdk::cs_player_t* player);
	int determine_air_side(resolver_context_t& ctx);
	int determine_freestand_side(resolver_context_t& ctx); // убрать до сюда
	...
	bool prepare_freestand(sdk::cs_player_t*);
	//void analyze_animation_layers(resolver_context_t& ctx)
		//// убрать
	bool jitter_fix(resolver_context_t&);
	bool air_fix(resolver_context_t&);
	void apply_edge(resolver_context_t&);
	void apply_side(resolver_context_t&);
