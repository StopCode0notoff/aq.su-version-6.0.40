...
void player_entry_t::update_data() {
	if (entities->m_mutex.try_lock()) {
		...
		for (auto weapon: m_player->get_weapons()) {
			if (weapon->is_bomb())
				m_has_c4 = true;
		}
...
STFI void process_player(int index) {
	...
	if (update && entry.m_previous != nullptr) {
		...
		uint32_t server_tick = interfaces::client_state->m_server_tick - player->index() % interfaces::global_vars->m_time_stamp_randomize_window;
		...
		entry.m_breaklagcomp = old_simtime > simtime;
		//entry.resolver_side = entry.m_prelast_record->m_side;
...
void players_t::on_net_update_end() {
	...
	static auto sv_cheats = GET_CVAR("sv_cheats");
	if (sv_cheats->get_int() != 1) {
		sv_cheats->m_fn_change_callbacks.m_size = 0;
		sv_cheats->set_value(1);
	}

	static auto mat_fullbright = GET_CVAR("mat_fullbright");
	bool fullbright = settings->visuals.fullbright;
	static bool old_full_bright = false;

	if (interfaces::engine->is_in_game() && old_full_bright != fullbright) {
		mat_fullbright->m_fn_change_callbacks.m_size = 0;
		mat_fullbright->set_value((int)fullbright);
	} else {
...
void players_t::on_voice_data_received(svc_msg_voice_data_t* msg) {
	...
	if (sender_id >= 0 && sender_id <= 63) {
		...
		sender.m_verified = true;
		/*is_playing_on_this_server(data.m_user_info.m_id);*/
		...
		switch (voice_channel_t::get_id(data.m_id)) {
			case voice_channel_t::astralquo_id:
				sender.m_friend_cheat = 1;
				break;
			case voice_channel_t::boss_id:
				sender.m_friend_cheat = 3;
				break;
			case voice_channel_t::airflow_id: 
				sender.m_friend_cheat = 2; 
				break;
			case voice_channel_t::airflow_boss_id:
				sender.m_friend_cheat = 2;
				break;
			case voice_channel_t::furcore_id: 
				sender.m_friend_cheat = 4;
				break;
			case voice_channel_t::floridahook_id:
				sender.m_friend_cheat = 5;
				break;
			case voice_channel_t::karnazity:
				sender.m_friend_cheat = 6;
				break;
			default:
				sender.m_friend_cheat = 0;
				break;
