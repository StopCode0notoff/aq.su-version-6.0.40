...
namespace hooks::client {
	...
	STFI void skybox_changer() {
		...
		const auto sky_name = STR("sky_lunacy");
	...
	void sunset_mode() {
		...
		bool sunset = (settings->visuals.sunsetmode.enable);
		...
		static auto cl_csm_rot_override = GET_CVAR("cl_csm_rot_override");
		static auto cl_csm_max_shadow_dist = GET_CVAR("cl_csm_max_shadow_dist");
		static auto cl_csm_rot_x = GET_CVAR("cl_csm_rot_x");
		static auto cl_csm_rot_y = GET_CVAR("cl_csm_rot_y");

		if (old_sunset_mode != sunset) {
			cl_csm_rot_override->m_fn_change_callbacks.m_size = 0;
			cl_csm_max_shadow_dist->m_fn_change_callbacks.m_size = 0;

			cl_csm_rot_override->set_value((int)sunset);
			cl_csm_max_shadow_dist->set_value(sunset ? 10000.f : -1.f);
			old_sunset_mode = sunset;
		}

		if (old_rot_x != settings->visuals.sunsetmode.sunset_angle_x) {
			cl_csm_rot_x->m_fn_change_callbacks.m_size = 0;

			cl_csm_rot_x->set_value(settings->visuals.sunsetmode.sunset_angle_x);
			old_rot_x = settings->visuals.sunsetmode.sunset_angle_x;
		}

		if (old_rot_y != settings->visuals.sunsetmode.sunset_angle_y) {
			cl_csm_rot_y->m_fn_change_callbacks.m_size = 0;

			cl_csm_rot_y->set_value(settings->visuals.sunsetmode.sunset_angle_y);
			old_rot_y = settings->visuals.sunsetmode.sunset_angle_y;
		}
	}
