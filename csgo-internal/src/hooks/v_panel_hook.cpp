			...
			if (!render::can_render || !gui::resources::downloaded) {
				if (!render::current_load_stage_name.empty()) {
					...
					if (render::total_load_stages.load() != 0) {
						...
						surface_render::filled_rect({ w * 0.5f - bar_size.x * 0.5f, h * 0.5f - bar_size.y * 0.5f + 32.f }, { bar_size.x * render::current_load_animated, bar_size.y }, color_t{ 0, 60, 255 }); //old color_t{ 254, 103, 49 }); | new color_t{ 0, 60, 255 }); setting up...
					} else
						surface_render::draw_string(verdana, { w * 0.5f, h * 0.5f }, color_t{}, render::centered,
