...
namespace NS_GUI {
	void resources::create_textures(IDirect3DDevice9* device, std::function<void()> callback) {
		...
		for (auto& name: files_to_create) {
			...
			texture = new render::gif_t{ utils::read_file_bin(std::vformat(STR("C:\\ProgramData\\astralquo\\assets\\{}.png"), std::make_format_args(name))) };

			//D3DXCreateTextureFromFileA(device, std::vformat(STR("C:\\astralquo\\assets\\{}.png"), std::make_format_args(name)).c_str(), (LPDIRECT3DTEXTURE9*)&texture);
		}
