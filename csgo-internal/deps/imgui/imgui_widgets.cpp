...
bool ImGui::ButtonEx_(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, int type) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
    ...
    switch (type) {
        case 1:
			inside_col[0] = ImColor(40, 40, 40, 255), inside_col[1] = ImColor(40, 40, 40, 255), inside_col[2] = ImColor(40, 40, 40, 255), inside_col[3] = ImColor(40, 40, 40, 255);
			break;
        case 2:
			inside_col[0] = ImColor(0, 51, 255, 255);   //0, 51, 255
			inside_col[1] = ImColor(101, 255, 0, 255); //101, 255, 0
			inside_col[2] = ImColor(101, 255, 0, 255);	//101, 255, 0
			inside_col[3] = ImColor(0, 51, 255, 255);   //0, 51, 255
			break;
	}

	draw->AddRectFilledMultiColor(bb.Min, bb.Max, inside_col[0], inside_col[1], inside_col[2], inside_col[3]);
...
bool ImGui::ButtonExTabs(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	// Render
	ImU32 inside_col[4] = { ImColor(0, 255, 250, 5) /*topleft*/,
							ImColor(89, 0, 255, 5) /*topright*/,
							ImColor(89, 0, 255, 5) /*botright*/,
							ImColor(0, 255, 250, 5) /*botleft*/ };
    ...
    { //при нажатии
		...
		inside_col[0] = ImColor(0, 255, 250, 5); //(91, 210, 0, 255);
	    inside_col[1] = ImColor(39, 0, 200, 5);  //inside_col[1] = ImColor(0, 35, 200, 255); //(128, 0, 255, 255);
		inside_col[2] = ImColor(39, 0, 200, 5);  //
		inside_col[3] = ImColor(0, 215, 210, 5);
    } else {
		inside_col[0] = ImColor(0, 215, 210, 5); 
		inside_col[1] = ImColor(39, 0, 200, 5);	  
		inside_col[2] = ImColor(39, 0, 200, 5);	  
		inside_col[3] = ImColor(0, 215, 210, 5);
    }
    //цвета кнопок
	...
	draw->AddRectFilledMultiColor(bb.Min, bb.Max, inside_col[0], inside_col[1], inside_col[2], inside_col[3]);
	//draw->AddRect(bb.Min, bb.Max, outside_solid_col, 1, 0, 2.f);
...
bool ImGui::ButtonColorChanger(ImTextureID texture, std::string label, const ImVec2& size) {
		//return ButtonEx(label, size_arg, ImGuiButtonFlags_None);
	return ImageButton(texture, label, size);
}

bool ImGui::ButtonLS(const char* label, const ImVec2& size_arg, int type) {
	return ButtonEx_(label, size_arg, ImGuiButtonFlags_None, type);
}

bool ImGui::ButtonTabs(const char* label, const ImVec2& size_arg) {
	return ButtonExTabs(label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    // Render
	const ImU32 col = GetColorU32(ImGuiCol_ImageButton);
    RenderNavHighlight(bb, id);
...
bool ImGui::ImageButton(ImTextureID user_texture_id, std::string label, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
        return false;

    std::string final_string = label + "#image";

    // Default to using texture ID as ID. User can still push string/integer prefixes.
    PushID((void*)(intptr_t)user_texture_id);
    const ImGuiID id = window->GetID(final_string.c_str());
    PopID();
...
bool ImGui::Checkbox(const char* label, bool* v) {
	...
	ImU32 col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.2f, 0.2f, 0.2f, 1.f), ImVec4(0.0f, 0.17f, 0.75f, 1.f), t)); //цвет кнопок цвет тумблера цвет переключателя цвет (1.f, 0.55f, 0.23f, 1.0f)

	draw_list->AddRectFilled(p, ImVec2(p.x + width, p.y + height), col_bg, height * 0.5f);
	draw_list->AddCircleFilled(ImVec2(p.x + radius + t * (width - radius * 2.0f), p.y + radius), radius - 1.5f, ImGui::GetColorU32(ImLerp(ImVec4(0.6f, 0.6f, 0.6f, 1.0f), ImVec4(1.0f, 1.0f, 1.0f, 1.0f), t)));
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    // Render shape
	const ImU32 frame_col = GetColorU32(hovered ? ImVec4(0.1f, 0.1f, 0.1f, 1.f) : ImVec4(0, 0, 0, 1.f));
    ...
    {
		ImU32 bg_col = GetColorU32((popup_open || hovered) ? ImVec4(0.2f, 0.2f, 0.2f, 1.f) : ImVec4(0.1f, 0.1f, 0.1f, 1.f)); //arrow down
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Draw frame
	const ImU32 frame_col = GetColorU32(/*g.ActiveId == id ? */ImVec4(0.41f, 0.f, 1.f, 1.f) /*:*/ /*hovered ? ImVec4(0.1f, 0.1f, 0.1f, 1.f)*/ //ЦВЕТ КНОПОК  ImVec4(1.f, 0.5f, 0.3f, 1.f) а точнее слайдер
										/*:*/ /* ImVec4(0.1f, 0.1f, 0.1f, 1.f)*/);
    RenderNavHighlight(frame_bb, id);
	RenderFrame(frame_bb.Min + ImVec2(5, 4), frame_bb.Max - ImVec2(5, 4), frame_col, true, 3.f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
		window->DrawList->AddRectFilled(grab_bb.Min - ImVec2(-1, 2), grab_bb.Max + ImVec2(-1, 2), GetColorU32(/*g.ActiveId == id ? */ ImVec4(1.f, 1.f, 1.f, 1.f) /*: ImVec4(0.1f, 0.1f, 0.1f, 1.f)*/), 3.f);
    ...
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
	RenderTextClipped(frame_bb.Min + ImVec2(100, 0), frame_bb.Max + ImVec2(100, 0), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
    {
		const ImU32 col = GetColorU32((held && hovered) ? ImVec4(0.25f, 0.25f, 0.25f, 1.f) : hovered ? ImVec4(0.2f, 0.2f, 0.2f, 1.f)
