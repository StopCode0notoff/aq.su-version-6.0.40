...
namespace sdk {
	...
	struct base_animating_t : base_entity_t {
		...
		OFFSET(last_setup_bone_time, float&, 0x2928); 
	...
	struct cs_player_t : base_player_t {
		...
		OFFSET_PTR(my_weapons, base_handle_t, 0x2E08);
		OFFSET(next_attack, float&, 0x2D80);
		...
		__forceinline void invalidate_bone_cache(int framecount) {
			...
			*(uintptr_t*)((DWORD)this + 0x2928) = 0xFF7FFFFF; //*(uintptr_t*)((DWORD)this + 0x2924) = 0xFF7FFFFF;
