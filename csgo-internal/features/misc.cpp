...
STFI void thirdperson() {
	...
	if (globals->m_local->is_alive() && interfaces::client_state->m_delta_tick > 0) {
		...
		auto distance = settings->visuals.thirdperson_dist;
...
void misc_t::on_render_start() {
	...
	auto weapon_debug_spread_show = GET_CVAR("weapon_debug_spread_show");
	...
	if (globals->m_local != nullptr) {
		...
		if (globals->m_local->is_alive()) {
			weapon_debug_spread_show->set_value(settings->visuals.force_crosshair && !globals->m_local->scoped() ? 3 : 0);
