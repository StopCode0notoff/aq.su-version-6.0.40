...
#include "../../utils/nt.hpp"
  ...
  namespace hvh {
	...
	STFI void modify_eye_position(cs_player_t* player, const matrix3x4_t* bones, vec3d& pos) {
		...
		#ifdef _DEBUG
			//c//onst auto state = players->m_local_player.m_real_state;
			//if (state == nullptr)
			//	return;
#endif // _DEBUG
	...
	vec3d aimbot_t::get_shoot_position(std::optional<vec3d> override_angle) {
		...
		*(vec3d*)((DWORD)globals->m_local + deadflag + 4) = angle;
		...
		if (globals->m_local->setup_bones(bone_matrix, 128, 0, interfaces::global_vars->m_curtime)) {
			vec3d head_pos = vec3d(bone_matrix[8][0][3], bone_matrix[8][1][3], bone_matrix[8][2][3]);
		}
	...
	std::vector<e_hitbox> aimbot_t::get_hitboxes_to_scan(bool primary) {
		...
		if (primary) {
			if (!hotkeys->force_body.m_active && aimbot->m_settings->hitboxes.at(0))
		...
		if (!hotkeys->force_body.m_active && aimbot->m_settings->hitboxes.at(6)) {
		...
		if (!hotkeys->force_body.m_active && aimbot->m_settings->hitboxes.at(7)) {
			hitboxes.emplace_back(e_hitbox::left_foot);
			hitboxes.emplace_back(e_hitbox::right_foot);
		}

		if (!hotkeys->force_body.m_active && aimbot->m_settings->hitboxes.at(8)) {
	...
	STFI lag_record_t* select_record(lag_record_t* first, lag_record_t* last) {
		...
		int first_damage = get_estimated_total_damage(first);
		int last_damage = get_estimated_total_damage(last);

		if (first_damage == last_damage) {

			return first->m_simulation_time < last->m_simulation_time ? first : last;
		}

		    if (first_damage != last_damage) {
			return first_damage > last_damage ? first : last;
		}

		float time_diff = last->m_simulation_time - first->m_simulation_time;
		if (time_diff < 0.2f) {
			return last;
		}
	...
	STFI lag_record_t* select_record(aimbot_t::data_t& data) {
		...
		lag_record_t* first = valid_records.front();
		lag_record_t* last = valid_records.back();

		if (valid_records.size() == 1)
			return first;


		float yaw_delta = fabs(math::normalize_yaw(last->m_eye_angles.y - first->m_eye_angles.y));
		if (yaw_delta > 30.0f) {
			last->m_resolver.m_mode.m_value |= e_resolver_mode::jitter; 
		}
	...
	STFI void reclamp_bones(lag_record_t* record, const vec3d& origin, const vec3d& eye_angles, matrix3x4_t* bones) {
		...
		float desync_delta = fabs(math::normalize_yaw(record->m_eye_angles.y - backup_record.m_eye_angles.y));
		//if (desync_delta > 25.0f) {
		//	record->m_resolver.m_mode.m_value |= e_resolver_mode::jitter; 
		//}
	...
	float get_edge_yaw(cs_player_t* player, const vec3d& origin) {
		if (player->velocity().length() > 5.0f || !player->flags().has(fl_onground))
			return player->eye_angles().y;

		static const float edge_check_distance = 32.0f;
		trace_t trace;
		trace_filter_t filter;
		filter.m_skip = player;

		float closest_wall_angle = -1.0f;
		for (float angle = 0.0f; angle < 360.0f; angle += 30.0f) {
			vec3d forward;
			math::angle_vectors(vec3d(0.0f, angle, 0.0f), forward);

			vec3d start = origin;
			vec3d end = start + (forward * edge_check_distance);

			ray_t ray;
			ray.init(start, end);
			interfaces::traces->trace_ray(ray, MASK_PLAYERSOLID, &filter, &trace);

			if (trace.m_fraction < 0.2f) {
				closest_wall_angle = angle;
				break;
			}
		}

		if (closest_wall_angle == -1.0f)
			return player->eye_angles().y; 

		return closest_wall_angle + 180.0f; 
	}
	...
	STFI void scan_player(aimbot_t::data_t& data) {
		...
		if (data.m_player->velocity().length() < 2.0f && data.m_player->flags().has(fl_onground)) {
			float edge_yaw = get_edge_yaw(data.m_player, data.m_record->m_origin);
			if (fabs(math::normalize_yaw(data.m_record->m_eye_angles.y - edge_yaw)) < 15.0f) {
				data.m_record->m_resolver.m_mode.m_value |= e_resolver_mode::edge;
			}
		}


		if (fabs(math::normalize_yaw(record->m_eye_angles.y - data.m_player->eye_angles().y)) > 35.0f) {
			record->m_resolver.m_mode.m_value |= e_resolver_mode::jitter;
		}

		// FakeLag wrong detect
		//if (record->m_simulation_time < data.m_player->simtime() - TICKS_TO_TIME(1)) {
		//	record->m_resolver.m_mode.m_value |= e_resolver_mode::fake_lag;
		//}

		
		///float desync_delta = fabs(math::normalize_yaw(record->m_eye_angles.y - data.m_player->eye_angles().y));
		//if (desync_delta > 40.0f) {
		//	record->m_resolver.m_mode.m_value |= e_resolver_mode::fake_yaw;
		//}

		if (!data.m_player->flags().has(fl_onground) || data.m_player->flags().has(fl_ducking)) {
			data.m_record->apply(data.m_record->m_bones);
		}

		if (fabs(math::normalize_yaw(data.m_record->m_eye_angles.y - data.m_player->eye_angles().y)) > 50.0f) {
			data.m_record->m_resolver.m_mode.m_value ^= e_resolver_mode::jitter; 
		}

		float desync_yaw = math::normalize_yaw(data.m_record->m_eye_angles.y - data.m_player->eye_angles().y);

		//std::string resolver_log = dformat(
			//	STR("Resolver Yaw: {:.2f} (Delta: {:.2f})"),
			//	data.m_record->m_eye_angles.y, fabs(desync_yaw));

		//cheat_logs->add_info(resolver_log);
	...
	STFI auto& create_data(int i, user_cmd_t* cmd, cs_player_t* player) {
		...
		data.m_tickcount = cmd->m_tickcount; //data.m_tickcount = cmd->m_tickcount;
	...
	STFI std::string hitbox_to_string(e_hitbox hitbox) {
		switch (hitbox) {
			...
			//case e_hitbox::neck: return STR("neck"); блядская шея
	...
	STFI bool shoot(user_cmd_t* cmd, aimbot_t::data_t& data) {
		...
		if (!exploits->m_in_shift) {
			...
			has_lethal_point_far = ranges::any_of(peek_state->m_players, [](const auto& p) { return p.m_has_lethal_point; }); // убрать
		...
		if ((has_lethal_point_far && !has_lethal_point_near) && !best_point.m_lethal) // убрать
			return true; // убрать
		...
		#ifdef _DEBUG
		//aimbot->m_eye_position = globals->m_eye_position; // убрать
		#endif //_DEBUG
		...
		if (cmd->m_buttons.has(e_buttons::in_attack)) {
			cmd->m_tickcount = TIME_TO_TICKS(data.m_record->m_simulation_time + lerp_time()); //cmd->m_tickcount = data.m_tickcount;
			...
			int bt = std::max<int>(0, interfaces::client_state->m_server_tick - TIME_TO_TICKS(data.m_record->m_simulation_time));

			float real_yaw = math::normalize_yaw(data.m_player->eye_angles().y);
			float fake_yaw = math::normalize_yaw(data.m_record->m_eye_angles.y);
			float desync_angle = fabs(math::normalize_yaw(real_yaw - fake_yaw));
			//desync_angle = min(desync_angle, 59.0f);
			//desync_angle = std::clamp(desync_angle, -59.0f, 59.0f);

			const auto message_colored = dformat(STR("[$1astralquo$6] $2Fired shot $3{} $2in $3{} $2for $3{}hp$2; $3{}%$2; $3sp: {}-{}-{}$2; $3bt: {}t$2; r: $3{}\n"), data.m_player->name(),
			...
			//for_each_extra_safe(data, [&data](auto* bones) { // убрать
			...
			aimbot->m_on_weapon_fire = [message_colored, message_cheatlog, best_point,
				...
				#ifdef _DEBUG
				draw_capsule(record->m_player, record->m_bones, {}, 4.f); // убрать
						draw_capsule(record->m_player, record->m_inversed_bones, { 255, 35, 35 }, 4.f);
						draw_capsule(record->m_player, record->m_unresolved_bones, { 35, 35, 255 }, 4.f); // убрать
				#endif
			};
			...
			#ifdef _DEBUG
			for (int i = 0; i < data.m_points.size(); ++i) { // убрать
	...
	STFI void run_aimbot(user_cmd_t* cmd) {
		...
		for (int i = 0; i < 64; ++i) {
			...
			if (data.m_command_number == cmd->m_command_number) {
				if (shoot(cmd, data)) {
					//if (!cmd->m_buttons.has(in_attack) && data.m_can_stop && aimbot->m_should_predictive_stop && !aimbot->m_autostop_called && _can_stop) // убрать
					//	movement->slow_walk(globals->m_cmd, globals->m_original_angle, 3.4f); // убрать
	...
	STFI void prepare_autostop(user_cmd_t* cmd) {
		...
		if (aimbot->m_settings->quick_stop && !globals->m_local->flags().has(fl_onground)) {

		//movement->slow_walk(cmd, globals->m_original_angle, 3.4f);
